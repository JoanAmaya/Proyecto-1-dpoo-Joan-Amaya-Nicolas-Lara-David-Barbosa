...
/*
              - Especificar rutas de archivos relativas o absolutas
              - la clase User tiene accesos por defecto si no se especifica una ruta
              - ser bastante específico al momento de ingresar formatos Horas y Fechas
              - Un administrador debió haber ingresado al sistema antes de un recepcionista y a su vez un recepcionista antes que un empleado
              - Tener cuidado al ingresar CC o TI en la creación de grupos 
              -
             */
public class UI {
        ...
        while (in_login)
        {
            ...
            if (USER.isAdministrator(login, passwd)) {
                ...
                if (admin == null) print("Habitaciones no cargadas");
                    ...
                    if (receptionist == null) receptionist = new Receptionist(desocupadas);
                    receptionistOptions();
                    in_login = false;
                }
            }
            ...
            switch (Integer.parseInt(input("Opción: ")))
            {
                case 1 -> {
                    ...
                    receptionist.crearReserva(
                            LocalDate.parse(input("fecha inicio de la reserva formato YYYY-MM-DD")),
                            LocalDate.parse(input("fecha final de la reserva formato YYYY-MM-DD")),
                            grupo
                    );
                ...
                case 4 -> {
                    Factura factura = new Factura("src/log.txt", receptionist.getReservasC(receptionist.huespedes()));
